/**
Forked from https://github.com/Auz/Bug
Localized to fit a Kintone environment.
**/

"use strict";

//-----
//Define according to your domain and app info
const appID = 22;
const statusname = "In review"; 
const username = "will";

//-----
//Define according to your domain locale (default US)
...
//-----
//Search for queried records and release the bugs!

//var searchquery = statusfield + "=" + "\"" + statusname + "\"";
//var searchquery = statusfield + "=" + "\"" + statusname + "\"" + " and " + assigneefield + " in " + "(\"USER\",\"" + username + "\")";
var searchquery = statusfield + "=" + "\"" + statusname + "\"" + " and " + assigneefield + " in " + "(" + username + ")";
var body = {
	"app": appID,
	"query": searchquery,
	"fields":[statusfield,assigneefield]
}

kintone.api(kintone.api.url('/k/v1/records',true), 'GET', body).then(function(resp) {
	//Release the bugs and spiders
//resp.records[0][assigneefield].value[0].code == username 
	if (resp.records.length > 0){
			new BugController({
			'minBugs':30 * resp.records.length, 
			'maxBugs':30 * resp.records.length, 
			'mouseOver':'die',
			'speed':10
			});

			new SpiderController({
			'minBugs':5 * resp.records.length, 
			'maxBugs':2 * resp.records.length, 
			'mouseOver':'die',
			'speed':10
			});
	}
